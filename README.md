...
- RNGD는 **LLM 스택**이라 이 브랜치의 정식 runtime 의미는 텍스트 생성(프롬프트 → 텍스트)입니다. numpy 추론이 아닙니다.
- 권장 public API는 `build_unified_LLM` / `create_runtime_LLM` / `generate_LLM` / `destroy_runtime_LLM` 입니다. `infer_LLM`은 호환 alias입니다.
