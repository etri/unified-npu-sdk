...
DOCKERFILE_PATH="${DOCKER_DIR}/docker.warboy.unified"

DOCKER_DEVICE_ARGS=()
DOCKER_TOOL_MOUNTS=()

print_usage() {
  echo "Usage: $0 [-n <container_name>] [--workspace <repo_path>] [--base-image <image>] [--pytorch-index-url <url>] [--furiosa-sdk-version <ver>] [--furiosa-pip-index <url>] [--device <node>]"
  echo ""
  echo "Options:"
  echo "  -n, --name    Container name (default: furiosa-only)"
...
echo "  python3 -c \"import torch, torchvision; print('torch=', torch.__version__, 'torchvision=', torchvision.__version__)\""
echo "  python3 -c \"import onnx; print('onnx=', onnx.__version__)\""
echo "  python3 examples/prepare_warboy_quantized_onnx.py --help"
